...
const PostBar = ({ imgurl }) => {
  ...
  const createPost = async () => {
    ...
    try {
      ...
      setPostArray([{ ...newPost, docid: docRef.id }, ...postArray]); // Add new post to state
      setPostText('');
      setPostImgUrl(null);
    } catch (error) {
...
const likePost = async (item) => {
  try {
    const washingtonRef = doc(db, "posts", item.docid);
    const newLike = { likerUid: userObj.userUid, likerPic: userObj.imgUrl, username: userObj.username };
    ...
    const commentPost = async (item) => {
      try {
        const washingtonRef = doc(db, "posts", item.docid);
        const newComment = {
          commenterUid: userObj.userUid,
          commenterPic: userObj.imgUrl,
          username: userObj.username,
          commentText,
        };
